...
from KplaneTimeFormer import TimeFormer  # 确保 TimeFormer.py 在目录下


# [定义新的渲染函数]
def render_timeformer_pers_only(viewpoint_camera, pc, motion_net, time_offset, pipe, bg_color):
    """
    完全复刻原版 render_motion 的 Align 逻辑，并将 TimeFormer 作为最终残差叠加。
    """
    # 1. 准备工作
    ...
    # 1. 始终先执行个性化/对齐网络 (Personalized/Align Step)
    #    注意：这里输入必须是原始 xyz，不能加 time_offset，否则 HashGrid 会崩
    p_motion_preds = pc.neural_motion_grid(shifted_xyz, audio_feat, exp_feat)

    # 2. 对齐操作 (Align): 修改通用网络的输入
    #    原版逻辑：如果 align=True，通用网络使用的是“对齐后”的坐标
    #    在 pretrain 阶段，我们通常默认开启 align 机制
    xyz_for_general = shifted_xyz + p_motion_preds['p_xyz']

    # 3. 执行通用网络 (General Motion Step)
    #    输入的是被 p_xyz 修正过的坐标
    motion_preds = motion_net(xyz_for_general, audio_feat, exp_feat)

    # 4. 组合变形量 (Compose Deformations)
    d_xyz = motion_preds['d_xyz']
    d_scale = motion_preds['d_scale']
    d_rot = motion_preds['d_rot']

    # 叠加个性化结果 (原版逻辑)
    # d_xyz *= (1 + p_motion_preds['p_scale']) # 原版注释掉的，我们也注释掉
    d_xyz += p_motion_preds['d_xyz']
    d_scale += p_motion_preds['d_scale']
    d_rot += p_motion_preds['d_rot']

    # 应用对齐缩放 (原版 align=True 时的逻辑)
    d_xyz *= p_motion_preds['p_scale']

    # ================= [TimeFormer 介入] =================
    # 将 TimeFormer 的 Offset 作为最后的“补丁”加在最终位置上
    # 这样既不干扰 Align 逻辑，也不干扰 HashGrid 输入
    means3D = pc.get_xyz + d_xyz + time_offset

    # ====================================================

    means2D = screenspace_points
    opacity = pc.get_opacity  # + motion_preds['d_opa'] (原版注释掉了 opa 变化，我们也保持一致)
    ...
    # 5. 光栅化 (使用处理过的返回值解包)
    rendered_image, radii, rendered_depth, rendered_alpha, *rest = rasterizer(
        means3D=means3D,
        means2D=means2D,
        shs=shs,
        colors_precomp=None,
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from,
    ...
    # vars
    warm_step = 1500 * len(data_list)  # 预热阶段结束点
    ...
    # ================= [TimeFormer Setup] =================
    # 为每个场景(人物)初始化一个 TimeFormer
    time_former_list = []
    time_former_optimizers = []

    # [修改] 1. 补全 opt 参数，适配 K-Plane TimeFormer 的 training_args 接口
    ...
    # K-Plane 和 Transformer 结构参数
    opt.input_dims = 4  # (x,y,z,t)
    opt.output_dims = 3  # (dx,dy,dz)
    opt.nhead = 4  # Transformer Heads
    opt.dim_feedforward = 36  # Hidden dims (保持你原代码的 36)
    opt.num_layers = 1  # Layers

    # 学习率缩放 (TimeFormer 内部使用)
    if not hasattr(opt, 'spatial_lr_scale'): opt.spatial_lr_scale = 5.0
    ...
    # 对应 data_list 的数量创建 TimeFormer
    for _ in data_list:
        # 使用 opt 初始化，不再使用 kwargs
        tf = TimeFormer(opt).cuda()
        tf.train_setting(opt)  # 初始化优化器
        time_former_list.append(tf)
        time_former_optimizers.append(tf.optimizer)

    # TimeFormer 权重参数
    weight_t_loss = 0.8
    weight_t_loss = 0.8
    weight_tf_reg = 0.0001
    # ======================================================
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        # [新增] 获取当前场景对应的 TimeFormer
        cur_timeformer = time_former_list[cur_scene_idx]
        cur_timeformer.update_learning_rate(iteration)
        ...
        if iteration > lpips_start_iter:
            ...
            mouth_mask = (-max_pool(-max_pool(mouth_mask[None].float())))[0].bool()  # 两次最小操作实现形态学闭运算（填充小孔洞）

        # 头发掩码更新条件（特定训练阶段且非更新间隔 每七次迭代）
        hair_mask_iter = (warm_step < iteration < lpips_start_iter - 1000) and iteration % hair_mask_interval != 0

        # ================= [TimeFormer 介入逻辑] =================
        # 仅在预热结束后启用 (对应您说的 Phase 3)
        if iteration > warm_step:

            scene_radius = scene.cameras_extent
            if scene_radius < 0.1: scene_radius = 1.0

            # 归一化 XYZ
            norm_xyz = (gaussians.get_xyz.detach() / scene_radius).clamp(-1.0, 1.0)

            # 1. 构造 TimeFormer 输入 (XYZ, t)
            # 归一化时间 t
            total_frames = len(scene.getTrainCameras())
            norm_t = viewpoint_cam.uid / (total_frames - 1)
            norm_t = 2 * norm_t - 1

            t_embed = torch.ones((gaussians.get_xyz.shape[0], 1), device="cuda") * norm_t

            tf_input = torch.cat([norm_xyz, t_embed], dim=1).unsqueeze(0)  # [1, N, 4]

            # 2. 计算 Offset
            # TimeFormer 输出 [1, N, 3] -> Squeeze -> [N, 3]
            offset = cur_timeformer(tf_input).squeeze(0)

            # 3. 渲染 (使用新写的函数)
            # 注意：这里我们替换了原有的 render_pkg
            # 这种方式是单流训练(将TF直接加入主流程)，或者是您想要的双流？
            # 如果是双流(Aux Stream)，需要保留原 render_pkg 算主Loss，再算一遍 TF Loss。
            # 下面演示的是：直接把 TF 加进主流程 (Single Stream with TF)，
            # 或者您可以保留原 render_pkg，额外算一个 render_pkg_tf。

            # 为了不破坏原有逻辑，我们作为 Aux Stream (辅助分支) 运行
            # 这样主分支(image_white) 保持原样，TF分支(image_tf) 算额外Loss

            render_pkg_tf = render_timeformer_pers_only(
                viewpoint_cam, gaussians, motion_net, offset, pipe, background
            )
            image_tf = render_pkg_tf["render"]

            # 4. 计算 TF 分支 Loss (Masked L1)
            gt_image = viewpoint_cam.original_image.cuda() / 255.0
            loss_tf = l1_loss(image_tf * head_mask, gt_image * head_mask)

            # 5. Offset 正则化
            loss_reg = l1_loss(offset, torch.zeros_like(offset))

            # 将 TF Loss 加入总 Loss (需要先定义 loss 变量，通常在原有代码的 Loss 部分之后加)
            # 为了代码整洁，我们可以在下面的 Loss 计算部分统一加
        ...
        ## gt_image_white[:, mouth_mask] = background[:, None]  # 嘴部区域置为背景
        ...
        # [新增] 将 TimeFormer Loss 加入总 Loss
        if iteration > warm_step:
            loss += weight_t_loss * loss_tf
            loss += weight_tf_reg * loss_reg

            # 记录日志
            if tb_writer:
                tb_writer.add_scalar("loss_tf", loss_tf.item(), global_step=iteration)

        #############################################################
        # 新增：反向传播前，记录总 Loss 到 TensorBoard
        ...
        if iteration > warm_step:
            ...
            #############################################################
            # 新增：反向传播前，记录正则化Loss 到 TensorBoard
            if tb_writer is not None:
            ...
            if iteration > p_motion_start_iter:
                ...
                loss += contrast_loss
                #############################################################
                # 新增：反向传播前，记录NC Loss 到 TensorBoard
                if tb_writer is not None:
                    # [修复] 兼容单场景训练的情况（此时 contrast_loss 为 int 0）
                    c_loss_val = contrast_loss.item() if isinstance(contrast_loss, torch.Tensor) else contrast_loss
                    tb_writer.add_scalar("contrast_loss", c_loss_val, global_step=iteration)
                #############################################################
                '''
            # 嘴部注意力正则化
        ...
        with torch.no_grad():
            ...
            if iteration < opt.iterations:
                ...
                if iteration > warm_step:
                    cur_timeformer.optimizer.step()
                    cur_timeformer.optimizer.zero_grad()
                # 清零运动网络梯度
