...
def rasterize_gaussians(
    ...
    color, depth, norm, alpha, radii, pixels, extra = _RasterizeGaussians.apply(
        means3D,
        means2D,
        sh,
        colors_precomp,
    ...
    # 3, H, W
    return color, depth, norm, alpha, radii, pixels, extra
...
class _RasterizeGaussians(torch.autograd.Function):
    ...
    def forward(
        ...
        if raster_settings.debug:
            ...
            try:
                num_rendered, color, depth, norm, alpha, radii, pixels, extra, geomBuffer, binningBuffer, imgBuffer = _C.rasterize_gaussians(
                    *args)
        ...
        ctx.save_for_backward(colors_precomp, means3D, scales, rotations, cov3Ds_precomp, norm3Ds_precomp, radii,
                              extra_attrs, sh, geomBuffer, binningBuffer, imgBuffer, alpha)
        return color, depth, norm, alpha, radii, pixels, extra

    @staticmethod
    def backward(ctx, grad_out_color, grad_out_depth, grad_out_norm, grad_out_alpha, grad_radii, _, grad_out_extra):
        ...
        if raster_settings.debug:
            cpu_args = cpu_deep_copy_tuple(args)  # Copy them before they can be corrupted
            try:
                grad_means2D, grad_colors_precomp, grad_opacities, grad_means3D, grad_cov3Ds_precomp, grad_norm3Ds_precomp, grad_sh, grad_scales, grad_rotations, grad_extra_attrs = _C.rasterize_gaussians_backward(
                    *args)
            except Exception as ex:
