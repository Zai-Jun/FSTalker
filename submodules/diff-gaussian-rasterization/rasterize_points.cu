  ...
  const bool debug)
{
  ...
  torch::Tensor pixels = torch::zeros({P, 1}, means3D.options()); //定
  ...
  if(P != 0)
  {
    ...
    rendered = CudaRasterizer::Rasterizer::forward(
      ...
      pixels.contiguous().data<float>(), //定
      radii.contiguous().data<int>(),
      debug);
  }
  return std::make_tuple(rendered, out_color, out_depth, out_norm, out_alpha, radii, pixels, out_extra, geomBuffer, binningBuffer, imgBuffer);
}
