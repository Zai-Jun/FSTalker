...
def render(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, override_color = None):
    ...
    rendered_image, rendered_depth, rendered_norm, rendered_alpha, radii, pixels, extra = rasterizer(
        means3D = means3D,
        means2D = means2D,
        shs = shs,
...
def render_motion(viewpoint_camera, pc : GaussianModel, motion_net : MotionNetwork, pipe, bg_color : torch.Tensor, \
    ...
    if return_attn:
        ...
        rendered_attn, _, _, _, _, _, _ = rasterizer(
        ...
        if personalized:
            ...
            p_rendered_attn, _, _, _, _, _, _ = rasterizer(
                means3D = means3D.detach(),
                means2D = means2D,
                shs = None,
...
def render_motion_mouth_con(viewpoint_camera, pc : GaussianModel, motion_net : MouthMotionNetwork, pc_face : GaussianModel, motion_net_face : MotionNetwork, pipe, bg_color : torch.Tensor, \
    ...
    with torch.no_grad():
        ...
        # [新增] 从 GaussianModel 取出权重
        # 注意：pc.get_mouth_weight 拿到的是 [N, 1] 的 Tensor
        mouth_w = pc.get_mouth_weight

    motion_preds = motion_net(xyz, audio_feat, move_feat.detach(),mouth_weight=mouth_w)
    d_xyz = motion_preds['d_xyz']
    # d_rot = motion_preds['d_rot']
